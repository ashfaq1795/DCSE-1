...
r=10;
hold on;
th=0:pi/60:2*pi;                           %pi/60 sample rate (121)
...
H_digit=1;
for th=pi/6:pi/6:2*pi;     %will run 12 time. in each iteration 30 degre 
    ytemp=r*cos(th)+y;
    xtemp=r*sin(th)+x;
...
%print 4 lines in two consecutive hours
...
while(1) %non terminating condition because clock alway run.
    ...
    sec=c(1,3);  %assign second to sec veriable
    min=c(1,2);  %assign minute to min veriable
    if c(1,1)>12;
       hr=c(1,1)-12;
    else
        hr=c(1,1);
